...
export const fromImageWithAlphaChannel = async (
    ...
    if (img.colorMode === ColorMode.RGB) {
        mat = await new cv.Mat(buffer, img.height, img.width, cv.CV_8UC4).cvtColorAsync(cv.COLOR_RGBA2BGR);
    } else {
        mat = await new cv.Mat(buffer, img.height, img.width, cv.CV_8UC4).cvtColorAsync(cv.COLOR_BGRA2BGR);
    }
    if (roi) {
...
export const fromImageWithoutAlphaChannel = async (
    ...
    const buffer = Buffer.isBuffer(img.data) ? img.data : Buffer.from(img.data);
    if (img.colorMode === ColorMode.RGB) {
        mat = await new cv.Mat(buffer, img.height, img.width, cv.CV_8UC3).cvtColorAsync(cv.COLOR_RGB2BGR);
    } else {
        mat = new cv.Mat(buffer, img.height, img.width, cv.CV_8UC3);
    }
    if (roi) {
        return mat.getRegion(determineROI(img, roi));
    } else {
